    ...
    if (newQty > currentQty) {
      ...
      for (let i = 0; i < (newQty - currentQty); i++) {
        ...
        if (qtyMenu >= 1) {
          ...
          const tt = `select * from cart_item_modifier 
            ...
            if (rec['menuSetAdjustItemsId']) {
              const b1 = `
                SELECT 
                  ...
                  FROM menu AS m 
                  WHERE m.presence = 1 AND m.adjustItemsId = '${rec['menuSetAdjustItemsId']}';
              `;
              const [n1] = await db.query(b1);
              if (n1[0]['qty'] > 0) {
        ...
        for (const cartItem of cartItems) {
          ...
          `;
            ...
            if (isDouble[0]['total'] == 0) {
              ...
              VALUES (
                1, '${today()}', '${today()}',  0,
                '${cartId}',  ${cartItem['id']}, ${modifier['id']},
                '', ${modifier['price']},
                ${userId}, ${userId}
            )`;
                const [result] = await db.query(q);
                ...
                if (result.affectedRows === 0) {
                  ...
                  results.push({ status: cartItem['id']+' + '+modifier['id']+' updated', query: q, });
                }
                const q2 =
