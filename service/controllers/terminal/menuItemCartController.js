   ...
   try {
      ...
      const items = formattedRows.map(header => {
         const itemModifier = modifier
            ...
            .map(detail => ({ ...detail, total: header.total })); // Add qty from header to each modifier
...
exports.voidItem = async (req, res) => {
   ...
   try {
      for (const emp of cart) {
         ...
         for (const mod of modifier) {
            if (mod['checkBox'] == 1) {
               ...
               // DELETE KUSUS DISCOUNT sudah SO
               const q8 = `UPDATE cart_item_discount
                      SET
                        void = 1,
                        presence = 0,
                        updateDate = '${today()}',
      ...
      // delete permanet jika sudah void dan presence 0 DAN sendOrder kosong
      const tablesmulty = [
         'cart_item',
         'cart_item_modifier',
         'cart_item_discount',
      ...
      for (const table of tablesmulty) {
         const q = `
            DELETE FROM ${table}
            WHERE presence = 0 AND void = 1 AND sendOrder = '';
         `;
         await db.query(q);
      }
         ...
         if (checkBox == 1) {
           // console.log('totalItem', totalItem, 'with requiredItemTotal:', discountGroup['requiredItemTotal']);
            ...
            if (allowAdd == true) {
               ...
               const t1 = ` 
                  SELECT SUM(t1.debit - t1.credit)  AS 'totalAmount'
                  FROM (
               ...
               const q = `
               INSERT INTO cart_item_discount (
                  presence, inputDate, updateDate, void,
                  cartId, cartItemId, discountId, note,
                  rate, amount, 
                  debit, credit,
                  inputBy, updateBy
               )
               VALUES (
                  1, '${today()}', '${today()}',  0,
                  '${cartId}',  ${id},  ${discountGroup['id']},   '${discountGroup['name']}',  
                  ${discountGroup['discRate']}, ${discountGroup['discAmount']},
