...
exports.postCreate = async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO outlet_payment (presence, inputDate, outletId ) 
      VALUES (?, ?,?)`,
      [
        1,
        inputDate,
    ...
    res.status(201).json({
      error: false,
      inputDate: inputDate,
      message: 'outlet_payment created',
      outlet_paymentId: result.insertId
    });
  } catch (err) {
    console.error(err);
...
exports.postDelete = async (req, res) => {
  ...
  try {
    for (const emp of data) {
      ...
      const [result] = await db.query(
        'UPDATE outlet_payment SET presence = ?, updateDate = ? WHERE id = ?',
